...
export class CartService {
  ...
  constructor(private http:HttpClient,private menu:MenusService) {
  ...
  addToCart(item:any){
    ...
    if(doesExist){
      ...
      items.push({ "itemId": item.id, "quantity": doesExist.quantity +1,price:item.price,thumbnail:item.thumbnail,name:item.name })
      this.cart.set(items)
    }else{
